...
def attachment_file(path: str) -> Path | None:
...
def restore_missing_invoice_amounts(conn: sqlite3.Connection, lines: list[dict[str, Any]]) -> None:
    repaired = False
    for line in lines:
        if normalize_receipt_type(line.get("receipt_type")) != "数电发票" or line.get("invoice_amount") is not None:
            continue
        file_path = attachment_file(normalize_text(line.get("attachment_path")))
        if not file_path:
            continue
        try:
            amount = extract_pdf_amount(file_path)
        except Exception:
            app.logger.exception("Stored invoice amount recovery failed for line %s", line.get("id"))
            continue
        if amount is not None:
            conn.execute("UPDATE expense_lines SET invoice_amount = ? WHERE id = ?", (amount, line["id"]))
            line["invoice_amount"] = amount
            repaired = True
    if repaired:
        conn.commit()
...
def list_claims(conn: sqlite3.Connection, actor: dict[str, Any], scope: str = "auto") -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        restore_missing_invoice_amounts(conn, claim["lines"])
